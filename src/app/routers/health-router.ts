...
class HealthRouter extends BaseRouter {
  constructor(
    @inject(REQUEST_HANDLERS_DI_TYPES.HealthRequestHandler)
    private readonly healthRequestHandler: IRequestHandler,
  ) {
    super();
  }
  ...
  setupRoutes() {
    this.router
      .route('/')
      .get(this.healthRequestHandler.handler.bind(this.healthRequestHandler));
  }
}
