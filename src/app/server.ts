...
export const createServer = (container: Container) => {
  ...
  const currentUserMiddleware = container.get<ICurrentUserMiddleware>(
  ...
  const errorMiddleware = container.get<IErrorMiddleware>(
    MIDDLEWARES_DI_TYPES.ErrorMiddleware
  );
