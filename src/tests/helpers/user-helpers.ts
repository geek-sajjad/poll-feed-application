...
export const createTestUser = async (
  ...
  const idGenerator = TestEnvironment.container.get<IIDGenerator>(
    CORE_DI_TYPES.IDGenerator,
  );
  const userRepo = TestEnvironment.container.get<IUserRepository>(
    REPOSITORIES_DI_TYPES.UserRepository,
  );
  const encryptor = TestEnvironment.container.get<IEncryptor>(
    SERVICES_DI_TYPES.Encryptor,
  );
