...
import { PollRouter } from '@/app/routers/poll-router';

export const registerRouters = (containerBuilder: ContainerBuilder) => {
  const builder = new RoutersContainerBuilder(
    containerBuilder
  ).registerRouters();
...
class RoutersContainerBuilder {
  ...
  registerRouters() {
    this.containerBuilder.registerActions.push(container => {
      container
        .bind<BaseRouter>(ROUTERS_DI_TYPES.ApiRouter)
        .to(ApiRouter)
        .inSingletonScope();
    });
    this.containerBuilder.registerActions.push(container => {
      container
        .bind<BaseRouter>(ROUTERS_DI_TYPES.AuthRouter)
        .to(AuthRouter)
        .inSingletonScope();
    });
    this.containerBuilder.registerActions.push(container => {
      container
        .bind<BaseRouter>(ROUTERS_DI_TYPES.UsersRouter)
        .to(UsersRouter)
        .inSingletonScope();
    });
    this.containerBuilder.registerActions.push(container => {
      container
        .bind<BaseRouter>(ROUTERS_DI_TYPES.HealthRouter)
        .to(HealthRouter)
        .inSingletonScope();
    });
    ...
    this.containerBuilder.registerActions.push(container => {
      container
        .bind<BaseRouter>(ROUTERS_DI_TYPES.PollRouter)
        .to(PollRouter)
        .inSingletonScope();
    });
