...
export const registerServices = (
  ...
  if (options?.onlyDatabase) {
    builder = new ServicesContainerBuilder(
      containerBuilder
...
class ServicesContainerBuilder {
  ...
  private registerAuthServices() {
    ...
    this.containerBuilder.registerActions.push(container => {
      container
        .bind<IAuthenticator>(SERVICES_DI_TYPES.Authenticator)
        .toDynamicValue(
          () =>
            new JwtAuthenticator(
              config,
              container.get<IUserRepository>(
                REPOSITORIES_DI_TYPES.UserRepository
              )
            )
        )
        .inSingletonScope();
  ...
  private registerSecurityServices() {
    this.containerBuilder.registerActions.push(container => {
      container
        .bind<IEncryptor>(SERVICES_DI_TYPES.Encryptor)
        .toDynamicValue(
          () =>
            new BcryptEncryptor({
  ...
  private registerDatabaseService() {
    ...
    this.containerBuilder.registerActions.push(container => {
      container
        .bind<IDatabase>(SERVICES_DI_TYPES.Database)
        .toDynamicValue(() => new Database(config))
        .inSingletonScope();
    });
  ...
  private registerCacheService() {
    this.containerBuilder.registerActions.push(container => {
      container
        .bind<RedisCache>(SERVICES_DI_TYPES.Cache)
        .toDynamicValue(() => RedisCache.getInstance())
        .inSingletonScope();
    });
  ...
  private getDatabaseConfig(): DatabaseConfig {
    ...
    return {
      ...
      port: isTest
        ? mandatoryIntegerEnv('TEST_DB_PORT')
        : mandatoryIntegerEnv('DB_PORT'),
      ...
      entities: [
        env('TYPEORM_ENTITIES', 'src/infra/database/models/**/*.entity.ts'),
      ],
      migrations: [
