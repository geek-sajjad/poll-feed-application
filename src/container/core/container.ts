...
export const registerCore = (containerBuilder: ContainerBuilder) => {
  const builder = new CoreContainerBuilder(containerBuilder).registerCore();
...
class CoreContainerBuilder {
  ...
  registerCore() {
    this.registerLogger().registerIdGenerator().registerTime();

    return this.containerBuilder;
  }

  private registerLogger() {
    const loggerType = unionEnv(
      'LOGGER_TYPE',
      ['console', 'broken'],
      'console',
    );
    ...
    this.containerBuilder.registerActions.push((container) => {
      container
        .bind<ILogger>(CORE_DI_TYPES.Logger)
        .to(logger)
        .inSingletonScope();
    });
  ...
  private registerIdGenerator() {
    this.containerBuilder.registerActions.push((container) => {
      container
        .bind<IIDGenerator>(CORE_DI_TYPES.IDGenerator)
        .to(UUIDGenerator)
        .inSingletonScope();
    });
