...
import {
  ...
  type IAuthenticatedMiddleware,
} from '@/app/middlewares/authenticated-middleware';
...
export const registerMiddlewares = (containerBuilder: ContainerBuilder) => {
  const builder = new MiddlewaresContainerBuilder(
    containerBuilder,
  ).registerMiddlewares();
...
class MiddlewaresContainerBuilder {
  ...
  registerMiddlewares() {
    this.registerCurrentUserMiddleware()
  ...
  private registerCurrentUserMiddleware() {
    this.containerBuilder.registerActions.push((container) => {
      container
        .bind<ICurrentUserMiddleware>(
          MIDDLEWARES_DI_TYPES.CurrentUserMiddleware,
        )
        .to(CurrentUserMiddleware)
        .inRequestScope();
    });
  ...
  private registerAuthenticatedMiddleware() {
    this.containerBuilder.registerActions.push((container) => {
      container
        .bind<IAuthenticatedMiddleware>(
          MIDDLEWARES_DI_TYPES.AuthenticatedMiddleware,
        )
  ...
  private registerErrorMiddleware() {
    this.containerBuilder.registerActions.push((container) => {
      container
        .bind<IErrorMiddleware>(MIDDLEWARES_DI_TYPES.ErrorMiddleware)
        .to(ErrorMiddleware)
        .inRequestScope();
    });
